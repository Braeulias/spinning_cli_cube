...
fn calculate_x(i: f32, j: f32, k: f32, a: f32, b: f32, c: f32) -> f32 { //uses 3D Rotation Matrix
    j * a.sin() * b.sin() * c.cos() - k * a.cos() * b.sin() * c.cos()
        + j * a.cos() * c.sin() + k * a.sin() * c.sin()
        + i * b.cos() * c.cos()
}

fn calculate_y(i: f32, j: f32, k: f32, a: f32, b: f32, c: f32) -> f32 {  //uses 3D Rotation Matrix
    j * a.cos() * c.cos() + k * a.sin() * c.cos()
        - j * a.sin() * b.sin() * c.sin() + k * a.cos() * b.sin() * c.sin()
        - i * b.cos() * c.sin()
}

fn calculate_z(i: f32, j: f32, k: f32, a: f32, b: f32) -> f32 {  //uses 3D Rotation Matrix
...
fn calculate_for_surface(
    ...
    if xp < 0 || xp >= width as isize || yp < 0 || yp >= height as isize {
        eprintln!("Out of bounds: xp={}, yp={}, width={}, height={}", xp, yp, width, height);
        return;
    }
    ...
    if let Some(z_buf) = z_buffer.get_mut(idx) {
        if ooz > *z_buf { //checks if char is visible in rotating cube
            *z_buf = ooz;
    ...
    loop {
        ...
        while cube_x < cube_width {
            ...
            while cube_y < cube_width {
                ...
                calculate_for_surface(cube_x, cube_y, -cube_width, '@', &mut buffer, &mut z_buffer, width, height, a, b, c, distance_from_cam, k1, horizontel_offset);
                calculate_for_surface(cube_width, cube_y, cube_x, '$', &mut buffer, &mut z_buffer, width, height, a, b, c, distance_from_cam, k1, horizontel_offset);
                calculate_for_surface(-cube_width, cube_y, -cube_x, '~', &mut buffer, &mut z_buffer, width, height, a, b, c, distance_from_cam, k1, horizontel_offset);
                calculate_for_surface(cube_x, cube_width, cube_y, '+', &mut buffer, &mut z_buffer, width, height, a, b, c, distance_from_cam, k1, horizontel_offset);
                calculate_for_surface(cube_x, -cube_width, -cube_y, ';', &mut buffer, &mut z_buffer, width, height, a, b, c, distance_from_cam, k1, horizontel_offset);
